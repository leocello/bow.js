...
bow.request = {
    res: null,
    token: null,
    make: function make(url, data, headers) {
        this.res = {
            url: null,
            method: 'GET',
            headers: {},
    ...
    setHeaders: function setHeaders(headers, merge) {
    ...
    getJsonResponse: function getJsonResponse() {
...
bow.form = {
    getInputs: function getInputs(form) {
...
bow.modal = {
    ...
    container: {
        ...
        footer: function footer(content) {
            if (typeof content !== 'undefined') {
    ...
    open: function open(element) {
        ...
        if (url !== '') {
            ...
            if (typeof $(element).attr('data-size') !== 'undefined') {
                self.container.id.find('.modal-dialog').addClass($(element).attr('data-size'));
            }
            if (typeof $(element).attr('data-class') !== 'undefined') {
                self.container.id.find('.modal-dialog').addClass($(element).attr('data-class'));
            }
            if (typeof $(element).attr('data-style') !== 'undefined') {
                self.container.id.find('.modal-dialog').css($(element).attr('data-style'));
            }
            if (typeof $(element).attr('data-btn-save') !== 'undefined') {
                self.container.id.find('.btn-save').text($(element).attr('data-btn-save'));
            }
            self.load(element);
        }
    },
    reset: function reset() {
        // @TODO: Reset modal's content
        this.container.id.html($('#frmGlobalModal').html());
        this.container.unbind();
        this.container.bind();
    },
    loading: function loading(show) {
        // @TODO: control the loading HTML
        var self = this;
        ...
        bow.ready.add(function() {
            ...
            if (bow.utils.isPluginLoaded('validator')) {
                // Validation Binding
                self.container.body().find('form').validator().on('invalid.bs.validator', function(e) {
    ...
    load: function load(e) {
        // @TODO: load HTML content of provided url
        var self = this;
...
bow.include = {
    js: function js(url, onComplete, onError) {
        if (url === '') {
            return false;
        }
    ...
    css: function css(url, media, onComplete, onError) {
        if (url === '') {
            return false;
        }
...
bow.input = {
        ...
        if (typeof options.keyCode !== 'undefined') {
            ...
            $.each(validKeyCodes, function(i, item) {
                if (typeof item === 'number' && keyCode === item) {
                    ...
                    if (item.length === 2 && keyCode >= item[0] && keyCode <= item[1]) {
                        isValid = true;
                    }
                }
            });
    ...
    isInteger: function isInteger(options) {
        return this.isValid($.extend(options, {
            type: 'integer'
        }));
    },

    isFloat: function isFloat(options) {
        return this.isValid($.extend(options, {
            type: 'float'
        }));
    }
...
bow.utils = {
    typeHead: {
        getEngine: function getEngine(data, display, value, callback) {
            if (typeof Bloodhound === 'undefined') return null;
            return new Bloodhound({
                datumTokenizer: function datumTokenizer(o) {
    ...
    isPluginLoaded: function isPluginLoaded(name) {
...
bow.window = {
    open: function open(u, o) {
        var n = typeof o.name === 'undefined' ? '' : o.name,
            r = typeof o.replace === 'undefined' ? false : o.replace,
            o = bow.url.build(o, '');
        window.open(u, n, o, r);
    },
    openNewWindow: function openNewWindow(u, o) {
        return this.open(u, $.extend(o, {
            name: '_blank'
        }));
    },
    openParentWindow: function openParentWindow(u, o) {
        return this.open(u, $.extend(o, {
            name: '_parent'
        }));
    },
    openCurrentWindow: function openCurrentWindow(u, o) {
        return this.open(u, $.extend(o, {
            name: '_self'
        }));
    },
    openNewTab: function openNewTab(u, o) {
